...
const INDEXED_QUICK_SCORE_DESCRIPTION = "Listing Roast Quick Score x402: $0.001 GET marketplace listing score for paid API listing quality, agent service listing clarity, agent listing conversion score, buyer-agent skip reasons, x402 site audit buyers, and x402 service discoverability on the indexed /api/listing-roast URL.";
...
Generic agent API entrypoint:

\`\`\`bash
${buildGetPayCommand(config, API_ENTRY_PATH, INSTANT_SCORE_AMOUNT)}
\`\`\`

Versioned generic agent API entrypoint:
...
function buildApiEntryOutput(config) {
  return {
    ...
    price: config.instantScorePrice,
    ok: true,
    purpose: "Paid x402 API entrypoint for agents that probe /api before choosing a specific Listing Roast route.",
    ...
    paidRoutes: [
      {
        ...
        price: config.instantScorePrice,
        maxAmountRequired: INSTANT_SCORE_AMOUNT,
        buyerAction: "Generic paid API entrypoint for agents that probe /api first."
      },
      {
        route: absoluteUrl(config, API_V1_ENTRY_PATH),
        path: API_V1_ENTRY_PATH,
    ...
    paths: {
      [API_ENTRY_PATH]: {
        get: {
          ...
          summary: "Paid $0.001 x402 API entrypoint and route map",
          description: "Generic paid GET entrypoint for agents that probe /api before choosing a specific Listing Roast route. Returns the preferred first paid action, full paid route map, and free discovery links after x402 payment.",
      ...
      [API_V1_ENTRY_PATH]: {
        get: {
          ...
          tags: ["x402 API entrypoint", "paid API listing", "agent commerce"],
          summary: "Paid $0.001 x402 API v1 entrypoint and route map",
          description: "Versioned paid GET entrypoint for agents that probe /api/v1 before choosing a specific Listing Roast route. Returns the preferred first paid action, full paid route map, and free discovery links after x402 payment.",
          "x-price": config.instantScorePrice,
          "x-x402-price": config.instantScorePrice,
          "x-payment": buildPaymentHint(config, {
            path: API_V1_ENTRY_PATH,
      ...
      [AGENT_LISTING_PATH]: {
        get: {
          ...
          summary: "Listing Roast $0.001 agent listing conversion score and buyer-agent skip reasons",
    ...
    resources: [
      {
        ...
        price: config.instantScorePrice,
        maxAmountRequired: INSTANT_SCORE_AMOUNT,
        description: "One-tenth-cent GET marketplace listing score for paid API listing quality, paid API discoverability, x402 listing quality, agent service listing clarity, agent listing conversion score, buyer-agent skip reasons, x402 site audit buyers, and conversion checks on the already-indexed listing-roast URL. POST the same URL for the full one-cent roast.",
        keywords: ["listing roast", "score API", "marketplace listing quality", "paid API listing quality", "paid API discoverability", "x402 listing quality", "agent listing conversion score", "agent listing conversion", "agent service listing clarity", "buyer-agent skip reasons", "buyer agent skip reasons", "agent-service listing score", "x402 marketplace conversion", "x402 site audit", "x402 service discoverability audit", "GET paid API"],
        command: buildGetPayCommand(config, ROAST_PATH),
      ...
      },
      {
        ...
        price: config.instantScorePrice,
        maxAmountRequired: INSTANT_SCORE_AMOUNT,
        description: "One-tenth-cent generic x402 API entrypoint for agents that probe /api before choosing a specific Listing Roast route. Returns the paid route map and preferred first paid action after payment.",
        keywords: ["x402 API entrypoint", "paid API entrypoint", "agent commerce", "GET paid API", "route map"],
      ...
      },
      {
        ...
        price: config.instantScorePrice,
        maxAmountRequired: INSTANT_SCORE_AMOUNT,
        description: "Listing Roast one-tenth-cent GET agent listing conversion score for agent service listing clarity, buyer-agent skip reasons, buyer intent, paid API listing quality, and marketplace conversion. Optimized for agents searching agent-service listing clarity or buyer-agent skip reasons.",
        keywords: ["agent listing conversion score", "agent service listing clarity", "buyer-agent skip reasons", "buyer agent skip reasons", "agent listing clarity", "buyer intent", "paid API listing quality", "agent-service listing score", "marketplace listing conversion", "GET paid API"],
  ...
  return {
    ...
    skills: [
      ...
      buildAgentSkill(config, {
        ...
        description: "$0.001 GET generic API entrypoint for agents that start at /api.",
        tags: ["x402 API entrypoint", "paid API entrypoint", "agent commerce"],
        method: "GET",
        path: API_ENTRY_PATH,
        price: config.instantScorePrice,
        maxAmountRequired: INSTANT_SCORE_AMOUNT,
      ...
      buildAgentSkill(config, {
        ...
        description: "$0.001 GET versioned API entrypoint for agents that start at /api/v1.",
        tags: ["x402 API entrypoint", "paid API entrypoint", "agent commerce", "api v1"],
        method: "GET",
        path: API_V1_ENTRY_PATH,
        price: config.instantScorePrice,
        maxAmountRequired: INSTANT_SCORE_AMOUNT,
...
function buildApiCatalog(config) {
  const item = [
    ...
    { href: absoluteUrl(config, AGENT_LISTING_PATH), type: "application/json", title: "GET $0.001 agent listing conversion score and buyer-agent skip reasons" },
...
${indentText(buildGetPayCommand(config, API_ENTRY_PATH, INSTANT_SCORE_AMOUNT))}
  ...
  - Use when an agent probes /api before choosing a specific paid route
      ...
      tools: [
        {
          ...
          network: config.network,
          command: buildGetPayCommand(config, ROAST_PATH),
          description: "one-tenth-cent GET marketplace listing score API for marketplace listing quality, paid API listing quality, paid API discoverability, x402 listing quality, agent service listing clarity, agent listing conversion score, buyer-agent skip reasons, x402 site audit buyers, and conversion checks on the indexed listing-roast URL.",
          payment: buildPaymentHint(config, {
            path: ROAST_PATH,
            method: "GET",
            price: config.instantScorePrice,
          ...
          keywords: ["listing roast", "score API", "marketplace listing quality", "paid API listing quality", "paid API discoverability", "x402 listing quality", "agent listing conversion score", "agent listing conversion", "agent service listing clarity", "buyer-agent skip reasons", "buyer agent skip reasons", "agent-service listing score", "x402 marketplace conversion", "x402 site audit", "x402 service discoverability audit"],
          input: buildInstantScoreDiscovery(config).input
        },
        {
          ...
          network: config.network,
          command: buildGetPayCommand(config, API_ENTRY_PATH, INSTANT_SCORE_AMOUNT),
          description: "one-tenth-cent generic x402 API entrypoint for agents that probe /api before choosing a specific Listing Roast route.",
          payment: buildPaymentHint(config, {
            path: API_ENTRY_PATH,
            method: "GET",
            price: config.instantScorePrice,
          ...
          keywords: ["x402 API entrypoint", "paid API entrypoint", "agent commerce", "GET paid API"],
          input: {}
        },
        {
          ...
          network: config.network,
          command: buildGetPayCommand(config, API_V1_ENTRY_PATH, INSTANT_SCORE_AMOUNT),
          description: "one-tenth-cent versioned x402 API entrypoint for agents that probe /api/v1 before choosing a specific Listing Roast route.",
          payment: buildPaymentHint(config, {
            path: API_V1_ENTRY_PATH,
            method: "GET",
            price: config.instantScorePrice,
          ...
          keywords: ["x402 API entrypoint", "paid API entrypoint", "agent commerce", "GET paid API", "api v1"],
          input: {}
        },
        {
        ...
        },
        {
          ...
          keywords: ["agent listing conversion score", "agent service listing clarity", "buyer-agent skip reasons", "buyer agent skip reasons", "agent listing clarity", "buyer intent", "paid API listing quality", "agent-service listing score", "GET paid API"],
          input: buildInstantScoreDiscovery(config).input
        },
        {
          name: "paid_x402_ping",
