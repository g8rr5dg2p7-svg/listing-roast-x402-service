...
- Browser and hosted-agent clients can call the free handoff routes or paid x402 routes cross-origin. The service answers `OPTIONS` preflight requests without payment and exposes `Payment-Required`, `X-PAYMENT-RESPONSE`, and `Link` headers so JavaScript clients can read the x402 challenge before retrying with `X-PAYMENT`.
