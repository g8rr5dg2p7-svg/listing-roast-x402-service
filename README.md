...
- Coinbase Bazaar MCP handoff: `GET /coinbase-x402-bazaar-mcp-server`
...
- `GET /paid-api-listing-quality`, `GET /marketplace-listing-conversion-api`, `GET /marketplace-listing-conversion`, `GET /buyer-agent-skip-reasons`, `GET /agent-service-clarity`, `GET /agent-listing-conversion`, `GET /x402-discovery-audit`, `GET /x402-site-audit`, `GET /agentcore-x402-payments`, and `GET /coinbase-x402-bazaar-mcp-server` - free no-spend intent landing pages for buyer agents and crawlers.
- `GET/POST /api/pay-now`, checkout-style aliases `GET/POST /api/checkout`, `GET/POST /checkout`, `GET/POST /api/buy`, `GET/POST /buy`, `GET/POST /api/pay`, `GET/POST /pay`, `GET/POST /api/start`, `GET/POST /start`, `GET /api/paid-usage-proof`, `GET /api/pricing`, `GET /api/find`, and `GET/POST /api/route` - free no-spend buyer handoffs for intent-aware pay commands, wallet-backed paid-use proof, prices, task-to-route matching, and Agent402-style local route ranking before payment. Use `/api/pay-now?intent=buyer-agent%20skip%20reasons` or POST JSON such as `{"intent":"buyer-agent skip reasons"}` to get the exact existing paid route for a buyer task, and `/api/paid-usage-proof` to verify paid completions and the receiver-wallet snapshot before paying. These handoffs also expose `provenFirstPaidAction`, which points to the indexed $0.001 `GET /api/listing-roast` route with wallet-backed paid-use proof.
- Browser and hosted-agent clients can call the free handoff routes or paid x402 routes cross-origin. The service answers `OPTIONS` preflight requests without payment and exposes `Payment-Required`, `X-PAYMENT-RESPONSE`, and `Link` headers so JavaScript clients can read the x402 challenge before retrying with `X-PAYMENT`. The pay-now handoff, unpaid JSON body, x402 manifest, OpenAPI, agent-tools manifest, and agent card also include `browserPayment` metadata with the same preflight and exposed-header instructions. OpenAPI paid operations also include `x-payment-info`, the compact payment marker used by discovery scanners to identify fixed-price x402 routes.
