...
- Latest verified deployment: f496f5fe-6e6a-4bd9-9ba7-c1e311080f25.
- Latest live code commit: 226df8f Align root POST fallback preview.
- Latest GitHub release: directory-post-conversion-v1.
- Latest metadata version: 2026-06-21-directory-post-conversion-v73.
- Revenue truth as of the 2026-06-21T03:59Z live no-spend check: 2 wallet-confirmed paid completions, $0.002 registered gross revenue, receiver wallet 1.001 USDC / 1001000 units. No third paid completion has been confirmed yet.
...
- Root `POST /` is now clearer for stale directory traffic without promoting it as the primary path: live no-spend verification confirmed the root POST 402 challenge is still capped at 1000 units, now tells agents to pay it only if they already followed a stale generic directory snippet, previews the immediate quick score plus route map and paid-use proof, and points clean buyers to `GET /api/listing-roast`.
...
- Public MCP metadata now exposes the JSON-RPC tool cards directly in GET metadata and server-card responses: `/.well-known/mcp.json`, `/mcp.json`, and `/.well-known/mcp/server-card.json` list `search_resources`, `proxy_tool_call`, and `listing_roast_x402_handoff` first with no-spend call examples. Live no-spend verification on 2026-06-21T03:45Z confirmed v72 metadata, tool routing, unchanged cash-register baseline, and a valid 1000-unit 402 challenge for `/api/listing-roast`.
- Directory POST conversion handoff is live in the root POST unpaid challenge and JSON body. Live no-spend verification on 2026-06-21T03:59Z confirmed v73 metadata, a 1000-unit `POST /` 402 challenge, the `directoryPostConversionHint`, the body-level immediate quick-score route-map description, unchanged cash-register baseline, and receiver wallet 1.001 USDC / 1001000 units.
...
- Current public paid-use proof: 2 wallet-confirmed paid completions, $0.002 registered gross revenue, receiver wallet 1.001 USDC / 1001000 units. Post-deploy no-spend verification at 2026-06-21T03:59Z confirmed the v73 directory POST conversion handoff, the v72 public MCP tool-card handoff, the v71 MCP exact-intent handoff in `POST /mcp`, the v70 exact-intent command fields in free handoff routes, the v43 full-roast direct handoff in `/x402.json` and `/api/paid-usage-proof`, the v42 full-roast unpaid-challenge signal, v41 cheap-cap discovery handoff, v40 cached discovery wallet-proof handoff, v38 pay-now wallet-proof handoff, v37 proof-action handoff, v36 search-reality handoff, and health route; it did not create a new paid completion.
